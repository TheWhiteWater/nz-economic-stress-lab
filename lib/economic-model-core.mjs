...
import rbnzSnapshot from "../engine/data/snapshots/rbnz_model_inputs.2026-06-30.json" with { type: "json" };

export { assumptions, rbnzSnapshot };
...
export function applySnapshotToInputs(inputs, snapshot = rbnzSnapshot) {
  return {
    ...inputs,
    sourceMode: "rbnz_snapshot",
    sourceNote: `RBNZ snapshot ${snapshot.mortgage_book_anchor.date}: C35 mortgage book ${(
      snapshot.mortgage_book_anchor.value_nzd / NZD_PER_B
    ).toFixed(3)}bn; C30 LVR>80 flow ${(snapshot.new_lending_lvr_flow.lvr_above_80_share * 100).toFixed(
      1,
    )}%; B30 2y rate ${snapshot.mortgage_rate_snapshot.two_year.toFixed(2)}%.`,
    marketBn: snapshot.mortgage_book_anchor.value_nzd / NZD_PER_B,
    mortgageRateShockPp: Number(
      Math.max(
        0,
        snapshot.mortgage_rate_snapshot.two_year - assumptions.scenarios.severe.normal.mortgage_rate_shock_pp,
      ).toFixed(2),
    ),
  };
}
...
export function makeInitialInputs(scenarioName = defaultScenario, designName = defaultDesign) {
  ...
  return {
    ...
    sourceNote: "Illustrative placeholder assumptions. Use RBNZ snapshot mode to anchor market size and rate context to downloaded RBNZ data.",
    scenarioName,
    designName,
